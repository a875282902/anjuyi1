...
- (void)getActivityInfo{
    ...
    [HttpRequest POST:path parameters:dic success:^(id  _Nullable responseObject) {
        ...
        if ([responseObject[@"code"] integerValue] == 200) {
            weakSelf.activityInfo = [NSDictionary dictionaryWithDictionary:responseObject[@"datas"]];
...
- (void)shareSuccess{
    ...
    NSDictionary *dic = @{@"user_id":self.activityInfo[@"id"],@"id":self.activityInfo[@"id"],@"type":@"7"};
