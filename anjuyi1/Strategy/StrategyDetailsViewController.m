...
@property (nonatomic,strong)StrategyCommentView  *  commentV;
...
- (StrategyCommentView *)commentV{
    
    if (!_commentV) {
        _commentV = [[StrategyCommentView alloc] initWithFrame:CGRectMake(0, 0, KScreenWidth, KScreenHeight)];
        
        WKSELF;

        [_commentV setSelectShowDetails:^(NSString *eva_id) {
...
- (void)createphotoInfo{
    ...
    headerBackImage  = [Tools creatImage:CGRectMake(0, 0, KScreenWidth,  750.0*KScreenWidth/750.0)  url:self.photoInfo[@"img"] image:@""];
    [headerBackImage setContentMode:(UIViewContentModeScaleAspectFill)];
...
- (void)shareSuccess{
    ...
    NSDictionary *dic = @{@"user_id":self.photoInfo[@"member_info"][@"id"],@"id":self.photoInfo[@"strategy_id"],@"type":@"3"};
...
- (void)collectThisHouse:(UIButton *)sender{
    ...
    NSString *path = [NSString stringWithFormat:@"%@/strategy/member_collect",KURL];
    
    NSDictionary *header = @{@"token":UTOKEN};
    NSDictionary *dic = @{@"strategy_id":self.strategy_id};
...
- (void)likeThisHouse:(UIButton *)sender{
    ...
    NSString *path = [NSString stringWithFormat:@"%@/strategy/member_zan",KURL];
